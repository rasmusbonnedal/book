...
namespace ImGui {
bool ComboAutoSelectComplex(const char* label, char* buffer, int bufferlen, int* current_item, int* cancel_item,
                            bool (*items_getter)(void*, int, const char**), void* data, int items_count, ImGuiComboFlags flags);
}
...
bool ImGui::ComboAutoSelectComplex(const char* label, char* input, int inputlen, int* current_item, int* cancel_item,
                                   bool (*items_getter)(void*, int, const char**), void* data, int items_count, ImGuiComboFlags flags) {
    ...
    if (!ItemAdd(total_bb, popupId, &frame_bb, 0)) return false;
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton)) {
        ImRect arrow_bb(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max);
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col,
                        ImGuiDir_Down, 1.0f);
    }
    if (!popupIsAlreadyOpened) {
        ...
        bool openPopup = pressed || popupNeedsToBeOpened;
        openPopup = openPopup || (g.NavJustMovedToId == popupId);
        openPopup = openPopup || (g.NavActivateId == popupId);

        if (openPopup && !popupIsAlreadyOpened) {
            if (window->DC.NavLayerCurrent == 0) window->NavLastIds[0] = popupId;
    ...
    ImGuiActivateFlags old_flags = g.NavActivateFlags;
    g.NavActivateFlags &= ~ImGuiActivateFlags_TryToPreserveState;
    bool done = InputTextEx("##inputText", NULL, input, inputlen, ImVec2(0, 0),
                            /* ImGuiInputTextFlags_AutoSelectAll | */ ImGuiInputTextFlags_EnterReturnsTrue, NULL, NULL);
    g.NavActivateFlags = old_flags;
    PopItemWidth();
    ...
    ImGuiWindowFlags window_flags2 =
        ImGuiWindowFlags_NoNavInputs | ImGuiWindowFlags_NoNavFocus;  // 0; //ImGuiWindowFlags_HorizontalScrollbar
...
bool ImGui::ComboAutoSelect(const char* label, ImGui::ComboAutoSelectData& data, const std::vector<std::string>& items,
                            ImGuiComboFlags flags) {
    return ComboAutoSelectComplex(label, data.input, sizeof(data.input) - 1, &data.index, &data.cancel_index, vector_item_getter,
                                  (void*)&items, (int)items.size(), flags);
}
